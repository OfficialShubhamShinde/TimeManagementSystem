...
?>
<?php
$sqlForDoj = "SELECT doj FROM employees where empid = $id";
$querySqlForDoj = mysqli_query($conn, $sqlForDoj);
$rowsqlForDoj = mysqli_fetch_assoc($querySqlForDoj);
$start_date = $rowsqlForDoj['doj'];

$monday = date('Y-m-d', strtotime('last Monday', strtotime($start_date)));
$friday = date('Y-m-d', strtotime('next Friday', strtotime($start_date)));
...
.time-input::-webkit-datetime-edit-text:before {
    content: ".";
    position: absolute;
    right: 18px;
    bottom: 2px;
    ...
    <div class="wrapper">
        ...
        <div class="main">
            ...
            <main class="content">
                ...
                <br>
                <div class="container aceptNotAlert">
                    ...
                    <div class="table-responsive">
                        <table class="table table-bordered">
                            <thead>
                                <tr>
                                <?php
                                // Fetch the date of joining for the employee with the given ID
                                // Fetch the date of joining for the employee with the given ID
                                $sqlForDoj = "SELECT doj FROM employees where empid = $id";
                                $querySqlForDoj = mysqli_query($conn, $sqlForDoj);
                                $rowsqlForDoj = mysqli_fetch_assoc($querySqlForDoj);
                                ...
                                // Get the date of the first Monday and last Friday of the week containing the $start_date
                                $monday = date('Y-m-d', strtotime('last Monday', strtotime($start_date)));
                                $friday = date('Y-m-d', strtotime('next Friday', strtotime($start_date)));
                            ...
                            <tbody>
                                ...
                                <form action="<?php echo $_SERVER['PHP_SELF']; ?>#alert" method="post">
                                    <?php
                                    include '../_dbconnect.php';
                                    $id = $_SESSION['empUserId'];
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <input type="number" step="0.01" class="without_ampm form-control time-input" name="t1" id="time" <?php if ($fatchEditableOrNot['status'] == 1) {
                                                echo '';
                                            } else {
                                                echo 'disabled';
                                        ...
                                        <td>
                                            <input type="number" step="0.01" class="without_ampm form-control time-input" name="t2" <?php if ($fatchEditableOrNot['status'] == 1) {
                                                echo '';
                                            } else {
                                                echo 'disabled';
                                        ...
                                        <td>
                                        <input type="number" step="0.01" class="without_ampm form-control time-input" name="t3" <?php if ($fatchEditableOrNot['status'] == 1) {
                                            echo '';
                                        } else {
                                            echo 'disabled';
                    ...
                    <?php
                    $SqlForSendToInsertFromStart = "select fridaydate from activity where id = '$id' AND week = '$weekNo'";
                    $QueryForSendToInsertFromStart = mysqli_query($conn, $SqlForSendToInsertFromStart);
                    $facthsendToInsertFromStart = mysqli_fetch_assoc($QueryForSendToInsertFromStart);
                    $sendToInsertFromStart = $facthsendToInsertFromStart['fridaydate'];
                    ...
                    <nav>
                        <ul class="pagination justify-content-center">
                          ...
                          <li class="page-item">
                            <?php echo '<a class="page-link" href="addNextYearWorkStatus.php?id=' . $fatchempid . '&weekno=' . ($weekNo + 1) . '&sentdate=' . $last_date . '&lastfridaydate=' . $sendToInsertFromStart . '">Next Week</a>'; ?>
                          </li>
                        </ul>
                      </nav>
                      
                    <form action="addNextYearWorkStatus.php" type="POST">
                    </form>
                </div>
